...
export default async function AdminWarehouseReportScrapPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  for (const l of logs) {
    ...
    if (!prev) by.set(l.productId, { productId: l.productId, sku: l.sku || null, name: l.productName || null, unit: l.unit, qty, cost });
    else {
      prev.qty += qty;
      prev.cost += cost;
    }
  ...
  const qs = new URLSearchParams({ w: String(w), from: from.toISOString(), to: to.toISOString(), report: "scrap" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Отчёт: брак</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-4 bg-slate-950 border-b border-slate-800 text-gray-400 text-sm">Списания по дефекту</div>
