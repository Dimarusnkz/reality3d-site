...
### 3. Чат (Chat)
- **Реализация**: Поллинг (5 сек для активного чата) с инкрементальной загрузкой.
- **Пагинация**: Курсорная (\`id\` сообщения). Загружается по 50 сообщений.
- **Типы**: Публичные сообщения и внутренние (\`isInternal\`) для сотрудников.
